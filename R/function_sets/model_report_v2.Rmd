...
post_processing_script_path <<- file.path(r_path,"function_sets","functional_PP.R")
source_smr_function_path <<- file.path(r_path,"function_sets","source_SMR_function.R")
perl_script_path <<-file.path(r_path,"function_sets","smr_multiple_users.pl")
...
```{r, NSE, echo = FALSE}
NSE_value <<- nse_Q(combined_data = VaM_data)
```

```{r, KGE, echo = FALSE}
KGE_value <<- kge_Q(combined_data = VaM_data)
```

## Model Performance Metrics:

| The NSE for stream flow from this run is: `r NSE_value`
| The KGE for stream flow from this run is: `r KGE_value`
