...
if ($operating_system eq 'darwin') {
    ...
    $home_dir_name = "home";
} elsif ($operating_system eq 'MSWin32') {
    $home_dir_name = "Users";
}
...
open ($WEATHER, '<', "/$home_dir_name/$user/Dropbox/SMR_R/raw_data/weather/noaa_pullman_mini.csv") || die "Can't open weather file\n"; 
#print "\n|----- line 329 -----|\n";
...
#____________________________________________________________________________________
  ...
  if ($doy == 365) { # need to make 365 eventually once markdown is set up
    ...
    open(OUT, ">>", "/$home_dir_name/$user/Dropbox/SMR_R/raw_data/smr_output/MFC_mass_balance_$user.csv") || die "Can't open output file\n";
